...
// @description    Swanalekha Transliteration tool for Malayalam
// ==/UserScript==
...
if (widget.getAttribute('bound')=='true')
{
	return;
}

widget.setAttribute('bound', 'true');
...
widget.removeEventListener('keypress', keypressDisabled, true);
widget.removeEventListener("keypress", keypressEnabled, true);
widget.addEventListener("keypress", keypressEnabled, true);
...
widget.removeEventListener('keypress', keypressEnabled, true);
widget.removeEventListener("keypress", keypressDisabled, true);
widget.addEventListener("keypress", keypressDisabled, true);
...
function bindAllTextElements(doc) {
	
	var ta=doc.getElementsByTagName('textarea');
 
	for(var i=0;i < ta.length;++i){
		//addCheckbox(ta[i]);
//alert('binding '+ta[i].name);
		bindSwanalekha(ta[i]);
	}
	var tb=doc.getElementsByTagName('input');
	for(var i=0;i < tb.length;++i){	
	 type = tb[i].getAttribute('type'); 	
		if ( type == 'text' || type == null) { 
			//alert('binding '+tb[i].name);
			bindSwanalekha(tb[i]);
		}
	}	
...
bindAllTextElements(document);
 }
//addLoadEvent(bindAllTextElements);
bindAllTextElements(document);

document.addEventListener('keypress', docClick, true);
/************ Swanalekha code ends here **********************/
